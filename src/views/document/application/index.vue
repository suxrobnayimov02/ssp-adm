      ...
      <div class="m-2">
         <b-row>
            ...
            >
               ...
               />
               <div class="w-50 mr-1" style="margin-top: -5px">
                  <form-select
                     :options="ApplicationFormingTypeList"
                     v-model="filter.applicationFormingTypeId"
         ...
         <template #cell(action)="{item}">
            <div class="text-center">
               ...
               <b-link
                  ...
                  :to="{ name : 'EditApplication',params : {id : item.id} }"
               ...
               <b-link
                  v-if="$can('ApplicationDelete', 'permissions')"
...
export default {
      ...
      return {
         ...
         fields: [
            ...
            },
            {
               ...
               label: this.$t('applicationclassification'),
               thClass: 'text-left',
               sortable: true
            },
            {
   ...
   methods: {
      Create() {
         if (this.filter.applicationFormingTypeId) {
            this.$router.push({
               ...
               query: {
                  applicationFormingTypeId: this.filter.applicationFormingTypeId
               }
            });
         } else {
            this.makeToast(`${this.$t('applicationFormingType')} ${this.$t('NotSelect')}`, 'danger');
         }
      },
      DbClick(item) {
         this.$router.push({ name: 'ApplicationEdit', params: { id: item.id } });
      },
      SortChange(data) {
      ...
      Refresh() {
         this.isBusy = true;
         ApplicationService.GetList(this.filter).then(res => {
