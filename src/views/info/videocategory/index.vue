      ...
      <div class="m-2">
         <b-row>
            ...
            >
               ...
               <b-button variant="primary" :to="{ name: 'EditVideoCategory',params : {id : 0}}">
      ...
      >
         ...
         <template #cell(actions)="{item}">
            <div class="text-center">
               <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
               <b-link
                  v-if="$can('VideoCategoryEdit', 'permissions')"
                  :to="{ name : 'EditVideoCategory',params : {id : item.id} }"
               ...
               <b-link
                  v-if="$can('VideoCategoryDelete', 'permissions')"
...
export default {
   ...
   methods: {
      DbClick(item) {
         this.$router.push({ name: 'EditVideoCategory', params: { id: item.id } });
      },
      SortChange(data) {
      ...
      Delete(item) {
         ...
         VideoCategoryService.Delete(item.id)
            .then(res => {
      ...
      Refresh() {
         this.isBusy = true;
         VideoCategoryService.GetList(this.filter).then(res => {
