    ...
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button variant="primary" :to="{ name: 'EditRelativeDegree',params : {id : 0}}">
    ...
    >
      ...
      <template #cell(actions)="{item}">
        <div class="text-center">
          <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
          <b-link
            v-if="$can('RelativeDegreeEdit', 'permissions')"
            :to="{ name : 'EditRelativeDegree',params : {id : item.id} }"
          ...
          <b-link
            v-if="$can('RelativeDegreeDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      ...
      RelativeDegreeService.Delete(item.id)
        .then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      RelativeDegreeService.GetList(this.filter).then(res => {
