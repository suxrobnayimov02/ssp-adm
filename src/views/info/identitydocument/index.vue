    ...
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button variant="primary" :to="{ name: 'EditIdentityDocument',params : {id : 0}}">
    ...
    >
      ...
      <template #cell(actions)="{item}">
        <div class="text-center">
          <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
          <b-link
            v-if="$can('IdentityDocumentEdit', 'permissions')"
            :to="{ name : 'EdiIdentityDocument',params : {id : item.id} }"
          ...
          <b-link
            v-if="$can('IdentityDocumentDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      ...
      IdentityDocumentService.Delete(item.id)
        .then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      IdentityDocumentService.GetList(this.filter).then(res => {
