  ...
  <b-card no-body>
    <div class="m-2">
      <b-row>
        ...
        >
          <!-- <label>{{$t('prtnContractType')}}</label> -->
          <v-select
          ...
          <b-button variant="primary" :to="{ name: 'EditPrtnContractType',params : {id : 0}}">
    ...
    >
      ...
      <template #cell(actions)="{item}">
        <div class="text-center">
          <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
          <b-link
            v-if="$can('PrtnContractTypeEdit', 'permissions')"
            :to="{ name : 'EditPrtnContractType',params : {id : item.id} }"
          ...
          <b-link
            v-if="$can('PrtnContractTypeDelete', 'permissions')"
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {
          key: "employeeRangeFrom",
          label: this.$t("employeeRangeFrom"),
          thClass: "text-center",
          tdClass: "text-center",
          sortable: true
        },
  ...
  methods: {
    ...
    Delete(item) {
      ...
      PrtnContractTypeService.Delete(item.id)
        .then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      PrtnContractTypeService.GetList(this.filter).then(res => {
