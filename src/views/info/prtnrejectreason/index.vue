  ...
  <b-card no-body>
    <div class="m-2">
      <b-row>
        ...
        >
          <!-- <label>{{$t('PrtnRejectReason')}}</label> -->
          <v-select
          ...
          <b-button variant="primary" :to="{ name: 'EditPrtnRejectReason',params : {id : 0}}">
    ...
    >
      ...
      <template #cell(actions)="{item}">
        <div class="text-center">
          <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
          <b-link
            v-if="$can('PrtnRejectReasonEdit', 'permissions')"
            :to="{ name : 'EditPrtnRejectReason',params : {id : item.id} }"
          ...
          <b-link
            v-if="$can('PrtnRejectReasonDelete', 'permissions')"
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {
          key: "prtnContractTypeTable",
          label: this.$t("prtnContractTypeTable"),
          thClass: "text-center",
          tdClass: "text-center",
          sortable: true
        },
  ...
  methods: {
    ...
    Delete(item) {
      ...
      PrtnRejectReasonService.Delete(item.id)
        .then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      PrtnRejectReasonService.GetList(this.filter).then(res => {
