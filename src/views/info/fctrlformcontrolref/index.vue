      ...
      <div class="m-2">
         <b-row>
            ...
            >
               ...
               <b-button
                  variant="primary"
                  :to="{ name: 'EditFctrlFormControlRef',params : {id : 0}}"
               >
      ...
      >
         ...
         <template #cell(actions)="{item}">
            <div class="text-center">
               <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
               <b-link
                  v-if="$can('FormControlRefEdit', 'permissions')"
                  :to="{ name : 'EditFctrlFormControlRef',params : {id : item.id} }"
               ...
               <b-link
                  v-if="$can('FormControlRefDelete', 'permissions')"
...
export default {
   ...
   methods: {
      DbClick(item) {
         this.$router.push({ name: 'FormControlRefEdit', params: { id: item.id } });
      },
      SortChange(data) {
      ...
      Delete(item) {
         ...
         FctrlFormControlRefService.Delete(item.id)
            .then(res => {
      ...
      Refresh() {
         this.isBusy = true;
         FctrlFormControlRefService.GetList(this.filter).then(res => {
