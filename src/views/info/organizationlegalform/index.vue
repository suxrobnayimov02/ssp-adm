  ...
  <b-card no-body>
    <div class="m-2">
      <b-row>
        ...
        >
          <!-- <label>{{$t('OrganizationLegalForm')}}</label> -->
          <v-select
          ...
          <b-button variant="primary" :to="{ name: 'EditOrganizationLegalForm',params : {id : 0}}">
    ...
    >
      ...
      <template #cell(actions)="{item}">
        <div class="text-center">
          <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
          <b-link
            v-if="$can('OrganizationLegalFormEdit', 'permissions')"
            :to="{ name : 'EdiOrganizationLegalForm',params : {id : item.id} }"
          ...
          <b-link
            v-if="$can('OrganizationLegalFormDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      ...
      OrganizationLegalFormService.Delete(item.id)
        .then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      OrganizationLegalFormService.GetList(this.filter).then(res => {
