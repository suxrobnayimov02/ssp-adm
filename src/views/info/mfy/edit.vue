            ...
            <b-row>
               <b-col sm="12" md="3">
                  <div class="form-group">
                     <form-input v-model="Data.code" rules="required" :label="$t('kode')" />
                  </div>
               </b-col>
               ...
               <b-col sm="12" md="3">
                  <div class="form-group">
                     <form-input v-model="Data.normativeDoc" :label="$t('normativeDoc')" />
                  </div>
               </b-col>

               <b-col sm="12" md="2">
                  <b-form-group :label="$t('isCenterDistrict')" label-cols="10">
                     <b-form-checkbox v-model="Data.isCenterDistrict" switch inline />
                  </b-form-group>
               </b-col>

               <b-col sm="12" md="2">
                  <b-form-group :label="$t('isFarmfy')" label-cols="10">
                     <b-form-checkbox v-model="Data.isFarmfy" switch inline />
                  </b-form-group>
               </b-col>
            </b-row>
            <b-row class="mt-3">
...
export default {
   ...
   created() {
      ...
      MfyService.Get(this.$route.params.id).then(res => {
         this.Data = res.data;
      });
      ...
      SaveData() {
         this.$refs.ValidationDTO.validate().then(success => {
            if (success) {
               ...
               MfyService.Update(this.Data)
                  .then(res => {
                     this.makeToast(this.$t('SaveSuccess'), 'success');
                     this.$router.push({ name: 'Mfy' });
                  })
                  .catch(err => {
