      ...
      <div class="m-2">
         <b-row>
            ...
            >
               ...
               <b-button variant="primary" :to="{ name: 'EditVideoLesson',params : {id : 0}}">
      ...
      >
         ...
         <template #cell(actions)="{item}">
            <div class="text-center">
               <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
               <b-link
                  v-if="$can('VideoLessonEdit', 'permissions')"
                  :to="{ name : 'EditVideoLesson',params : {id : item.id} }"
               ...
               <b-link
                  v-if="$can('VideoLessonDelete', 'permissions')"
...
import VideoLessonService from '@/services/info/videolesson.service';
...
export default {
   ...
   methods: {
      DbClick(item) {
         this.$router.push({ name: 'EditVideoLesson', params: { id: item.id } });
      },
      SortChange(data) {
      ...
      Refresh() {
         this.isBusy = true;
         VideoLessonService.GetList(this.filter).then(res => {
