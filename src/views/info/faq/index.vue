      ...
      <div class="m-2">
         <b-row>
            ...
            >
               ...
               <b-button variant="primary" :to="{ name: 'EditFaq',params : {id : 0}}">
      ...
      >
         ...
         <template #cell(actions)="{item}">
            <div class="text-center">
               <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
               <b-link
                  v-if="$can('FaqEdit', 'permissions')"
                  :to="{ name : 'EditFaq',params : {id : item.id} }"
               ...
               <b-link
                  v-if="$can('FaqDelete', 'permissions')"
...
export default {
   ...
   methods: {
      DbClick(item) {
         this.$router.push({ name: 'EditFaq', params: { id: item.id } });
      },
      SortChange(data) {
      ...
      Refresh() {
         this.isBusy = true;
         FaqService.GetList(this.filter).then(res => {
