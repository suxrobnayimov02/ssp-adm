<template>
   <b-card no-body>
      <div class="m-2">
         <b-row>
            ...
            >
               ...
               <b-button variant="primary" :to="{ name: 'EditFctrlFormControl',params : {id : 0}}">
      ...
      >
         <template #cell(isRequired)="{ item }">
            <b-badge v-if="item.isRequired" variant="success">
               {{
               $t("yes")
               }}
            </b-badge>
         ...
         <template #cell(action)="{item}">
            <div class="text-center">
               <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
               <b-link
                  v-if="$can('FormControlEdit', 'permissions')"
                  :to="{ name : 'EditFctrlFormControl',params : {id : item.id} }"
               ...
               <b-link
                  v-if="$can('FormControlDelete', 'permissions')"
...
export default {
   ...
   data() {
      return {
         ...
         fields: [
            ...
            {
               key: 'formControlTypeName',
               label: this.$t('formControlType'),
               thClass: 'text-left',
               sortable: true
            },
   ...
   methods: {
      DbClick(item) {
         this.$router.push({ name: 'FormControlEdit', params: { id: item.id } });
      },
      SortChange(data) {
      ...
      Delete(item) {
         ...
         FctrlFormControlService.Delete(item.id)
            .then(res => {
      ...
      Refresh() {
         this.isBusy = true;
         FctrlFormControlService.GetList(this.filter).then(res => {
