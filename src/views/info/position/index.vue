    ...
    <div class="m-2">
      <b-row>
        ...
        >
          ...
          <b-button variant="primary" :to="{ name: 'EditPosition',params : {id : 0}}">
    ...
    >
      ...
      <template #cell(actions)="{item}">
        <div class="text-center">
          <!-- v-if="$store.state.auth.organizationtype !== 4 && $can('RoleEdit','permissions')" -->
          <b-link
            v-if="$can('PositionEdit', 'permissions')"
            :to="{ name : 'EditPosition',params : {id : item.id} }"
          ...
          <b-link
            v-if="$can('PositionDelete', 'permissions')"
...
export default {
  ...
  methods: {
    DbClick(item) {
      this.$router.push({ name: "EditPosition", params: { id: item.id } });
    },
    SortChange(data) {
    ...
    Delete(item) {
      ...
      PositionService.Delete(item.id)
        .then(res => {
    ...
    Refresh() {
      this.isBusy = true;
      PositionService.GetList(this.filter).then(res => {
